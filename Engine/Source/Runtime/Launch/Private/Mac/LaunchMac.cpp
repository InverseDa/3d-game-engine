...
static LE::Array<char> ReadShaderFile(const char* Filename)
{
    LE::Array<char> Buffer;
...
static bool IsValidSpirv(const LE::Array<char>& ByteCode, const char* ShaderLabel)
{
    if (ByteCode.IsEmpty())
    {
...
static LE::FRALBuffer* CreateTriangleVertexBuffer(LE::FRALDevice* Device)
{
    FSimpleVertex Vertices[3] = {
        { LE::Math::Vector3f( 0.0f,  0.5f, 0.0f), LE::Math::Vector3f(1.0f, 0.0f, 0.0f) },
        { LE::Math::Vector3f( 0.5f, -0.5f, 0.0f), LE::Math::Vector3f(0.0f, 1.0f, 0.0f) },
        { LE::Math::Vector3f(-0.5f, -0.5f, 0.0f), LE::Math::Vector3f(0.0f, 0.0f, 1.0f) },
    };
...
static void GetBackBufferExtent(LE::FRALSwapchain* Swapchain, uint32 FallbackWidth, uint32 FallbackHeight, uint32& OutWidth, uint32& OutHeight)
{
...
static bool SyncSwapchainToWindowSize(FPlatformWindow* Window, LE::FRALSwapchain* Swapchain, uint32& InOutWidth, uint32& InOutHeight)
{
